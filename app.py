...
def edit_contact(id):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('view_contacts_paginated', page=1))
    # Retrieve clients not linked to this contact
    available_clients = Client.query.filter(~Client.contacts.any(contact_id=contact.id)).all()
    return render_template('contact_form.html', contact=contact, editing=True, available_clients=available_clients)
